...
**Query (`ftmq/query/`)**: The central query language (see [Query language](#query-language-ftmqquery) below). Chainable `.where()`, `.order_by()`, `.aggregate()`, `.select()`; slicing `q[10:20]`; applied via `q.apply(entity)` / `q.apply_iter(entities)`, or compiled to SQL via `q.sql` / `q.compile(source)`.

**I/O (`ftmq/io.py`)**: `smart_read_proxies()` and `smart_write_proxies()` auto-detect source type (file, URL, store URI) and handle streaming. `smart_read_statements()` / `smart_write_statements()` are the same pair one level down, over raw statement streams (`csv` / `json` / `pack`) instead of entities - the store side of the resolver round trip.
...
**Projection.** `Query.select(*refs)` (`P` / `G` refs only, `QueryError` otherwise) restricts which properties a matching entity is read with - parallel to `where()` / `aggregate()`, never changing which entities match. SQL: a `prop` / `prop_type` row predicate on the statement fetch (`Sql._selection_clause`), always OR-ed with `prop = 'id'` so an entity holding none of the selected props still comes back; in memory: `Query._project` clones and pops, run last in `apply_iter` so filters, sort and aggregations see the full entity. Wire spelling `select=properties.title` / rql `select(...)` / dict `{"select": [...]}`.

Nodes compose with `&`, `|`, `~` into arbitrary boolean trees. `Query.where(*nodes)` AND-combines positional nodes; chained `.where()` also ANDs. An `Expr` canonicalizes its children on construction (`nodes.py:_normalize`): a non-negated sub-group of the same connector is spliced in, and a structurally equal child is dropped (`a & a` is `a`), so every consumer - the serializers, the SQL compiler and the in-memory evaluator - reads a deduplicated tree instead of normalizing on its own. Lookups are `field__comparator=value` (the comparator set lives in `ftmq/query/leaves.py:COMPARATORS`; in-memory semantics in `Leaf.match`, SQL translation in `Sql.get_expression`). Sorting is single-field (`order_by("name")` / `order_by("-date")`). Invalid queries raise `QueryError` (subclass of `ValueError`).
...
Package layout: `refs.py` (`Ref` hierarchy - one class per family - + `Year` + the `wire` / `ref_from_wire` field codec), `nodes.py` (`Expr` tree + `M`/`P`/`G`/`C` + `combine`), `leaves.py` (leaf classes + factories + `LeafDict` + `group_conjunction`, the co-reference rule), `aggregations.py` (`A` + `Agg` specs + in-memory `Aggregator`), `aleph.py` (Aleph URL-param bridge), `rql.py` (RQL string bridge), `sql.py` (`Sql` + `SqlSource`, see [SQL integration](#sql-integration-ftmqquerysqlpy)), `main.py` (`Query` + `Sort`), `exceptions.py` (`QueryError`). `__init__.py` only re-exports.
...
- `to_dict()` / `from_dict()` - lossless nested tree (any tree, plus aggregations / selection / sort / slice).
- `to_rql()` / `from_rql()` - [RQL](https://github.com/pjwerneck/pyrql) string (via `pyrql`): the only string surface carrying arbitrary `& | ~` nesting, plus aggregations via RQL's `sum` / `mean` / `count` / `aggregate(...)` operators and the projection via its `select(...)`; raises `QueryError` for comparators with no RQL equivalent (`null`, `startswith`, ...).
...
The param bridge maps `M`→`filter:schema|schemata|dataset|...`, `P`→`filter:properties.<name>`, `G`→`filter:group.<name>`, `C`→`filter:context.<name>`, `~`→`exclude:`, `__null`→`empty:`, aggregations→`metric:<func>=<field>` + `facet=<field>`, the projection→`select=<field>` (fields spelled exactly as the filter keys are). The grammar is bidirectional: params can query ftmq stores, and a `Query` can drive an OpenAleph-style API.
...
**Resolver / linker.** A store resolves entity ids through a nomenklatura `Linker` (the dedupe decisions). `store/base.py:get_resolver(uri)` is the read/write `Resolver` over a sql `resolver` table - the default for a store opened without a `linker` (its own database; a non-sql store gets an ephemeral in-memory one) - and it *loads its decisions into memory on construction*, since a freshly built `Resolver` resolves nothing until indexed. `get_linker(uri)` is the read-only counterpart used on a read path (the api's `FTMQ_API_RESOLVER_URI`): the same sql table, or an edge dump (`Resolver.dump()` json lines) read through anystore with no database at all. `SQLStore._iterate` overrides nomenklatura, which groups the statement row stream by `entity_id` while every select feeding it orders by / filters on `canonical_id` - that splits a merged cluster into one partial entity per referent. A linker resolves *ids*, not data: everything reads by the `canonical_id` column, so a store has to be resolved on write (the sql / lake writers stamp it; the memory store keeps the canonical id a statement already carries; an existing store is resolved by dumping its statements, stamping them with `nomenklatura apply-statements` and loading them back - `ftmq statements read` / `write`, which upsert in place because the statement id doesn't cover `canonical_id`). That load must *not* re-derive the canonical id from the target's linker, which every backend writer does: `io.smart_write_statements` points the store at a `PreservingLinker` (`store/base.py`) answering with the id the statement already carries, keyed on `entity_id` so entity-typed *values* (which the memory / leveldb writers resolve for their inverted index) still map to themselves. `Store.statements()` is the dump side, SQL-family only - reading via entities would synthesize a fresh `id` statement and regenerate cloned ones, so the dump would no longer upsert onto its own rows. Given a resolved store the linker is still needed on the read path for one thing - mapping a *referent* id to its canonical before the lookup (`api/store.py:View.get_entity`); filters, counts and aggregations read the resolved ids out of the data. Against an *un*resolved store nothing but that id mapping works, which is what `docs/api.md` requires of an api deployment.
...
`Sql` translates a `Query` into SQLAlchemy clauses; `SqlSource` describes what it compiles against (table, id column, optional partition pruning, optional row-level `base_filter`). Access via `query.sql` (default nomenklatura statement table), `query.compile(source)`, or `Sql(query, source, scope=...)` for a dataset-scoped view (what store views do). Partition pruning is a `prune={column: fn}` dict, one rule per partition column: `fn(query)` returns that column's possible values and `Sql` folds them into every compiled query as a `column IN (...)` row predicate (`Sql._prune_clauses`). A rule only runs for a flat positive conjunction (`_is_flat_and`) - under `~` / `|` / a repeated field a filter no longer restricts matching entities to its partitions - and whether the rule's *own* fields are positive is the rule's business (`prune_by_schema`, the schema -> partition mapper, skips a `not` / `not_in` schema comparator). The SQL and Lake stores own their `SqlSource` (`SQLStore.source`; the Lake store folds its `{"bucket": prune_by_schema(get_schema_bucket)}` rule and its view filter into every compiled query). Arbitrary boolean trees compile - see [SQL compilation semantics](#sql-compilation-semantics).

### Search (`ftmq/search/`)

Full-text "shallow search" stores for FtM entities. `logic.transform` flattens entities into `EntityDocument` search docs (names, rigour-based fingerprints via `ftmq.util.entity_fingerprints`, countries, dates, text blob); stores index those docs and answer `search(q, query=None)` / `autocomplete(q)`, where the optional `Query` is compiled by `store.base.get_filters` into a flat list of ANDed `FilterTerm`s over the three indexed fields (`datasets` / `schema` / `countries`; an is-a filter expands to the concrete schemata, a filter on any other field is dropped). A `not` / `not_in` comparator (or `~` around a single condition) becomes a *negated* term - do not read the polarity-less `Query.dataset_names` / `schemata_names` / `countries` accessors here, that is what made an `exclude:` filter select what it should exclude. A shape the term list cannot represent (cross-field OR, negated group, `ilike` and friends on an indexed field) raises `QueryError`, which the api turns into a 400. On a multi-valued field a negated term means "holds none of these values" (Aleph `exclude:` semantics), diverging from the in-memory / SQL reading of `not` as "holds some other value". Backends via `ftmq.search.store.get_store(uri=...)`: `sqlite://` (FTS5, no extra deps) and `tantivy://` / `memory://` (require the `search` extra, i.e. `tantivy`; lazily imported in the factory). Settings use the `FTMQ_SEARCH_` env prefix. The CLI lives in `ftmq/search/cli.py` as a typer sub-app (`ftmq search ...`); a bare query routes to its `search` command via `SearchDefaultGroup`.
...
Subcommands: `dataset`, `catalog`, `store`, `fragments`, `search`, `statements`, `aggregate`, `apply-dataset`. The `statements` group works on raw statement streams: `cast-types` (value normalization) plus `read` / `write` to move statements out of and into a store. The default-command plumbing (`DefaultCmdTyperGroup`) lives in `ftmq/cli_util.py` (not in the `ftmq.cli` package) so the search sub-app can subclass it without an import cycle.
